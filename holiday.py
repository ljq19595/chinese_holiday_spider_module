...
def get_holiday_url(year, html_doc):
    ...
    for td in td_list:
        ...
        search = re.search('国务院办公厅关于(\d+)年部分节假日安排的通知', title)
...
def match_holiday(html_doc, year=datetime.datetime.now().year + 1):
    html_doc = re.sub(r'<(/|)br(/|)>', '\n', html_doc.decode('utf-8')) # 将标签换行改为换行符。
    ...
    holiday = []
    for line in html:
        mat = re.search("^[一二三四五六七八九十]*?、(.*?)$", line.strip())
        if mat:
            desc = mat.group(1)
            name, plan = desc.split(u"：", 2)
            holiday_mat = re.search("(?P<holiday>\d+月\d+日.*?)放假", plan)
            workday_mat = re.search("[，。](?P<workday>\d+月\d+日[^，。]*?)上班", plan)
            ...
            for i in range(len(holiday_parse)):
                holiday.append(holiday_parse[i])
...
def parse_date(desc, year):
    mat_days = re.findall(
        "(?P<from_month>\d+)月(?P<from_day>\d+)日至(?P<to_month>\d+)月(?P<to_day>\d+)日", desc)
    if mat_days:
        days = []
        ...
        for mat in mat_days:
            ...
            for month in range(int(from_month), int(to_month) + 1):
                ...
                day_begin = 1
        return days
    mat_days = re.findall("(?P<month>\d+)月(?P<from>\d+)日至(?P<to>\d+)日", desc)
    if mat_days:
        days = []
        for mat in mat_days:
        ...
        return days
    mat_days = re.findall("(?P<month>\d+)月(?P<from>\d+)日", desc)
    if mat_days:
        days = []
        for mat in mat_days:
            month, day = mat
...
def get_holiday(year):
    ...
    if holiday_url:
        return {"holiday": match_holiday(get_url_html(holiday_url, 'utf-8'), year)}
    return None

def write_file(data, file):
    with open(file, "w",  encoding='utf-8') as f:
        f.write(str(data))
        print("write data to %s success" %file)

if __name__ == "__main__":
    year = datetime.datetime.now().year
    result_json = get_holiday(year)
    if(result_json):
        write_file(json.dumps(result_json), str(year)+".json")
    else:
        print("error read holiday data from internet")
